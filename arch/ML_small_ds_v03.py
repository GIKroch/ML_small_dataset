...
# ### 3.1. Feature Selection for Linear Model
...
data1.to_csv(path_or_buf='data1.csv')


# ### 3.2. Feature Selection for Linear Model with logarithmized dependent variable

# As we know from exploratory data analysis, the distribution of dependent variable SalePrice is rather log-normal. Thus, logarithmizing it could improve model accuracy and therefore we will do feature selection for logarithmized dependent variable seperately. 
...
data2 = data[["SalePrice"] + list(final.keys())]
data2.to_csv(path_or_buf='data2.csv')


# ## 3.2 Regression

# In our analysis we will be using the Train Validate Test data split approach as we believe it helps achieve pure and reliable evaluation of the model.
# Firstly, we create test and train+validate sets.
...
X_loglm = data2.loc[:, data1.columns != 'SalePrice']
y_loglm = np.log(data2.SalePrice)
X_loglm_train, X_loglm_test, y_loglm_train, y_loglm_test = train_test_split(X_loglm, y_loglm, test_size=0.3, random_state=42)
